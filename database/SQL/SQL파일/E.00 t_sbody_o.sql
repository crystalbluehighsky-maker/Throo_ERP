-- [거래] 공급업체 AP 보조부 미결데이터 테이블
CREATE TABLE t_sbody_o (
    -- 기본키 (PK) 구성
    comcd       VARCHAR(10)   NOT NULL, -- 회사코드
    suppcd      VARCHAR(10)   NOT NULL, -- 공급업체(Supplier) 코드
    mulky       CHAR(1)       NOT NULL, -- Multi Key (Down pay. 등)
    clrdt       DATE          NOT NULL, -- Clearing Date (미결 시 보통 빈값이나 PK로 설정됨)
    ...
    -- 전표 기본 정보
    docty       CHAR(2)       NOT NULL, -- 전표종류 (예: SI, SC)
    ...
    -- 복합 기본키 설정
    CONSTRAINT pk_t_sbody_o PRIMARY KEY (
        comcd, suppcd, mulky, clrdt, clrdoc, fisyr, docno, lineno
    )
);

COMMENT ON TABLE t_sbody_o IS '공급업체 AP 보조부 미결데이터 테이블';