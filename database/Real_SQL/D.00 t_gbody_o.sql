-- [거래] GL 보조부 미결 및 잔액 데이터 테이블
CREATE TABLE t_gbody_o (
    -- 기본키 (PK) 구성
    ...
    cscode      VARCHAR(10),            -- 고객/공급업체 코드 (정의서 cscode 기반)
    taxcd       CHAR(2),                -- 세금코드
    ...
    -- 복합 기본키 설정
    CONSTRAINT pk_t_gbody_o PRIMARY KEY (
        comcd, glmaster, mulky, clrdt, clrdoc, fisyr, docno, lineno
    )
);

COMMENT ON TABLE t_gbody_o IS 'GL 보조부 미결 및 잔액 데이터 테이블';